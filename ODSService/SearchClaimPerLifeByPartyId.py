...
from retrieve_claim_detail import retrieveClaimDetial
import yaml
# import pprint
...
class ClaimID(object):
    ...
    def __init__(self, companyId, claimId):
...
#     'User-Agent':'Mozilla/5.0'
# }
url = '{}/{}'.format(url,'rest/AIAService/claim/searchClaim/SearchClaimPerLifeByPartyId')
...
def callService():
    ...
    for p in party_list:
        try:
            oFileName = '{}_{}_{}_{}_{}.json'.format(output_path,'SearchClaimPerLifeByPartyId',p,'Resp',curDt)
            with io.open(oFileName,'a',encoding='utf8') as o:
                count += 1
                print('###{}###'.format(count))
                json_data_str = '{"partyId":"'+p+'","companyId":"1" ,"relationShip":"I" ' + dedupe_mode + '}'
                ...
                output = r.json()
                claimPerLifeList  = output['claimPerLifeList']
                #findUniqueClaimPerLifeList(claimPerLifeList)
                claim_list = findClaimPerLifeList(claimPerLifeList)
                retrieve_clm_detial = retrieveClaimDetial(cmic_config)
                retrieve_clm_detial.callService(claim_list,now)
                #output = r.json()
...
def findClaimPerLifeList(claimPerLifeList):
    claim_list = []
    import re
    for claimPerLife in claimPerLifeList:
        businessKey = claimPerLife['businessKey']
        mcs = re.search('^MCS', businessKey)
        if mcs!=None :
            claimNo = claimPerLife['claimNo']
            claimId = claimPerLife['claimId']
            print(f'{claimNo}, businessline {businessKey}, claimId:{claimId}')
            #claim_list.append(claimPerLife)
            #claimID = ClaimID("1", claimId)
            if claimId in claim_list:
                continue
            claim_list.append(claimId)
    return claim_list

def findUniqueClaimPerLifeList(claimPerLifeList):
    clm_id_Set = set()
    import re
    for claimPerLife in claimPerLifeList:
        businessKey = claimPerLife['businessKey']
        mcs = re.search('^MCS', businessKey)
        if mcs!=None :
            claim_id = claimPerLife['claimId']
            clm_id_Set.add(claim_id)
            print(f'claimId:{claim_id}')
    return clm_id_Set
